...
for fqdn in ${fqdn_list[@]}; do
  ...
  #")
  observed_fqdn=$(ssh ${hostname} "
    echo '<RIBCL VERSION=\"2.0\"><LOGIN USER_LOGIN=\"Administrator\" PASSWORD=\"\"><SERVER_INFO MODE=\"read\"><GET_SERVER_FQDN /></SERVER_INFO></LOGIN></RIBCL>' | sudo hponcfg --input | grep SERVER_FQDN | cut -d '\"' -f 2
  ")
  if [ "${fqdn}" = "${observed_fqdn}" ]; then
    echo "✅ ${fqdn}"
  else
    ssh ${hostname} "
      echo '<RIBCL VERSION=\"2.0\"><LOGIN USER_LOGIN=\"Administrator\" PASSWORD=\"\"><SERVER_INFO MODE=\"write\"><SERVER_FQDN value=\"${fqdn}\"/></SERVER_INFO></LOGIN></RIBCL>' | sudo hponcfg --input &> /dev/null
    "
